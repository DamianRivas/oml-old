package open.caesar.oml.settings
...
class PreferenceValuesProvider implements IPreferenceValuesProvider {
	...
	static def IPreferenceValues createPreferenceValues(org.eclipse.emf.common.util.URI projectURI) {
		...
		val userSettings = userHome.resolve(".oml").resolve("oml.settings")
		...
		val workspaceSettings = Paths.get(workspaceDirectory).resolve("oml.settings")
		...
		val projectSettings = Paths.get(projectDirectory).resolve("oml.settings")
